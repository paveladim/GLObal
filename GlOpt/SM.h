...
public:
    double get_solution() { return maximum; }
    ...
    void doPivotting(int pivotRow, int pivotColumn) {
        ...
        vector<double> pivotRowVals(cols);//the column with the pivot

        vector<double> pivotColVals(rows);//the row with the pivot

        vector<double> rowNew(cols);//the row after processing the pivot value
